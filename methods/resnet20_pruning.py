...
if __name__ == "__main__":
    ...
    for i in range(0, 19):
        amount = i * 0.05
        model_pruned = torch.hub.load("chenyaofo/pytorch-cifar-models", "cifar10_resnet20", pretrained=True).to(device)
        get_model_info_extended(model_pruned, test_loader=test_loader)
        ...
        pruner = tp.pruner.MagnitudePruner(
            ...
            ignored_layers=[],  # layers you don't want to prune
            root_module_types=[torch.nn.Conv2d],  # types of modules you prune
        )
        ...
        model_pruned_retraind, avg_time,_,_ = train(model=model_pruned,
                                                    train_set=train_set,
                                                    val_set=val_set,
                                                    test_set=test_set,
                                                    run_name = f"resnet20/pruning/{int(100 * amount)}_percent",
                                                    epochs=epochs,
                                                    batch_size=256,
                                                    optimizer_name="SGD",
                                                    lr=lr,
        ...
        info_after = get_model_info_extended(model_pruned_retraind, test_loader, train_time_sec=avg_time)
        test_accuracy_after = info_after["test_accuracy"]

        log_rows.append([
            int(amount * 100),
            f"{model_size_MB:.3f}",
            f"{trainable_parameters / 1e3:.2f}",
            f"{infer_time:.2f}",
    ...
    headers = ["Pruning %", "Size (MB)", "Parameters (K)", "Inference Time (ms/image)",
               "Accuracy Before Fine-Tuning (%)", "Accuracy After Fine-Tuning (%)", "Learning Rate", "Epochs",
               "Epoch Time (s)"]

    csv_path = path_to_save("resnet20/pruning", "resnet20_pruning_results_summary_table.csv")
    ...
    plot_path1 = path_to_save("resnet20/pruning", "resnet20_pruning_results_summary_table.png")
    plt.title("ResNet-20  Pruning Results Summary Table", fontsize=12, weight='bold')
    ...
    plt.title('ResNet-20 Test Accuracy vs Pruning %')
    plt.xlabel('Pruning %')
    plt.ylabel('Test Accuracy')
    plt.legend()
    plt.grid(True)

    plot_path_acc = path_to_save("resnet20/pruning", "resnet20_accuracy_vs_pruning.png")
